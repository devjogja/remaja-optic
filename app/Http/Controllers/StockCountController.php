...
class StockCountController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (count($ezpos_product_list)) {
            $csvData = array('No; Item; Kode; Jumlah; Terhitung');
            $n = 1;
            foreach ($ezpos_product_list as $product) {
                $csvData[] = $n++ . ";" . $product->name . ";'" . $product->code . ";" . $product->qty . ";" . "";
            }
    ...
    public function stockDif($id)
    {
        ...
        while (!feof($file_handle)) {
            $cl = fgetcsv($file_handle);
            if ($cl && $i > 0) {
                $current_line = explode(';', $cl[0]);
                if ($current_line[3] != $current_line[4]) {
                    $current_line[2] = substr($current_line[2], 1);
                    $product[] = $current_line[1] . ' [' . $current_line[2] . ']';
                    $expected[] = $current_line[3];
                    $product_data = Product::where('code', $current_line[2])->first();
